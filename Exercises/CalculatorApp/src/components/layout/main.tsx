...
function reducer(state: State, action: Action) {
    ...
    switch (type) {
        case "Evaluate": 
            return { ...state, answer: eval(`${state.currentOperand}`) }
        ...
        case "Clear": 
            return { ...state, currentOperand: "", answer: null }
...
const Main = () => {
    const [state, dispatch] = useReducer(reducer, { currentOperand: "", answer: null });

    return (
        <div className="flex justify-center items-center h-screen">
            <main className="rounded-2xl p-2 space-y-3 text-gray-300 font-bold text-2xl">
                <div className="w-96 h-40 space-y-5 bg-[#121414] rounded-2xl overflow-hidden p-4 flex flex-col justify-center items-end">
                    <h2 className="text-3xl">{state.currentOperand}</h2>
                    <h3 className="text-xl text-gray-500">{state.answer}</h3>
                </div>
                <div className="border-b-2 border-gray-800 flex justify-end pr-3 pb-2">
                    <button className="" onClick={() => dispatch({ type: "Delete Digit" })}>
                ...
                <section className="grid grid-cols-4 gap-3">
                    ...
                    <OperationButton Text="/" BgColor="bg-[#132335]" Dispatch={() => dispatch({ type: "Add Digit", payload: { digit: "/" } }) } />
                    {["7", "8", "9"].map((num, i) => <DigitButton key={i} Text={num} BgColor="bg-[#171919]" Dispatch={() => dispatch({ type: "Add Digit", payload: { digit: num } })} />)}
                    <OperationButton BgColor="bg-[#132335]" Text="X" Dispatch={() => dispatch({ type: "Add Digit", payload: { digit: "*" } })} />
                    {["4", "5", "6"].map((num, i) => <DigitButton key={i} Text={num} BgColor="bg-[#171919]" Dispatch={() => dispatch({ type: "Add Digit", payload: { digit: num } })} />)}
                    <OperationButton BgColor="bg-[#132335]" Text="-" Dispatch={() => dispatch({ type: "Add Digit", payload: { digit: "-" } })} />
