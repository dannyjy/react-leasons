...
const AddTodo = () => {
    const [todo, setTodo] = useState<string[]>(JSON.parse(localStorage.getItem("tasks") as string) ?? []);

    function getData(formData: FormData){
        const { task } = Object.fromEntries(formData)
        if(task === null || task === '') return;
        setTodo((tasks) => [...tasks,task.toString()])
    }
  ...
  return (
    <div className="flex flex-col items-center"> 
        ...
        <ul>{todo.map((t,i) => <li key={i}>{t}</li>).reverse()}</ul>
    </div>
  )
}
